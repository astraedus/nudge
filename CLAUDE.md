...
Privacy-first app blocker with delay-to-open (breathing exercises before opening distracting apps), per-app daily time budgets, app groups, schedule-based rules, in-app feature blocking (YouTube Shorts, Instagram Reels, TikTok), Tab Vanish (covering Instagram's Reels tab outright while it is blocked), and grayscale mode. Zero internet permission. All data local.
...
Room DB version 11. Migrations: 1->2 (schedule/inapp/grayscale), 2->3 (userChangedMind), 3->4 (showCounter), 4->5 (autoKickAfter), 5->6 (showTimeRemaining, autoKickCooldownSeconds), 6->7 (webDomains), 7->8 (autoKickAfterMinutes), **8->9 (DROPS the dead `usage_events.durationMs` column, issue #22)**, 9->10 (`BlockRule.webBlockMode`, issue [#21](https://github.com/astraedus/nudge/issues/21); also repairs the rows that bug created), 10->11 (`BlockRule.tabVanish` / `followingSteer`, the Tab Vanish cover and the Instagram Following steer — see `docs/architecture/rules-and-features.md`).
...
| `docs/architecture/export-import.md` | Backup format (rules/groups/history/settings), per-entry failure policy, the import Strict-Mode gate, the save/share/import entry points, the DB 8→9 recreate | editing `data/export/`, `ui/backup/`, `ExportRulesUseCase`, `ImportRulesUseCase`, `applyImportedSettings`, `MIGRATION_8_9`, or adding a screen that offers backup |
