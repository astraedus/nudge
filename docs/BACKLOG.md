...
- [x] ~~**`isOwnAppWindowEvent` can never return true in production, so "the user opened Nudge" never clears the awareness overlays.**~~ **FIXED in v1.17.3** ([#33](https://github.com/astraedus/nudge/issues/33)). `shouldClearForOwnPackageEvent` tested `className?.startsWith(ownPackageName)` with the **applicationId** `dev.astraedus.nudge` while every class this app ships is `com.astraedus.nudge.*`, so the prefix never matched and the `clearOverlays(packageName, "own_app_window")` branch was unreachable for months. Its unit tests passed throughout because they supplied their own `ownPackageName` (`PassthroughTest`'s `OWN_PACKAGE = "com.astraedus.nudge"`) — the trap recorded in `tasks/lessons.md` (2026-09-14). **What the fix is**: "is this class ours" is now ONE predicate, `BlockLaunchGate.isOwnNudgeClass`, shared with the positive, exact-class `isOwnMainAppWindow` from #36; the service derives the namespace from a real class (`BuildConfig::class.java.packageName`) instead of naming it; and `NudgeIdentity` gives the whole test tree one derived source for the applicationId and the namespace, with `OwnClassNamespaceContractTest` asserting they are DIFFERENT strings so nobody can collapse them again. Four suites had been feeding the namespace in as the app's package and reading the same literal back; they now read the real values. The clear does not double-fire: the awareness overlays' own windows classify as `ForegroundSignal.AwarenessOverlay` and return above the `OwnUi` branch (#41), so a pill cannot order itself hidden.
...
- [x] ~~**F7 — no `CoroutineExceptionHandler` on `NudgeAccessibilityService.serviceScope`**~~ **RESOLVED in v1.17.3.** Every block evaluation, `UsageEvent` write and DataStore collect runs there; `SupervisorJob` stops sibling cancellation and does NOT stop an unhandled exception reaching the thread's default handler, which kills the process and the a11y service with it. The scope is now a `util/CrashSafeScope`, which is `RecordWalkAwayUseCase`'s pattern extracted so it can be tested: `CrashSafeScopeTest` asserts a throwing child leaves its siblings, the scope and later work alone and is logged at e-level with the coroutine's name, and carries the counterfactual that gives those assertions meaning — the same coroutine on a bare `SupervisorJob` scope DOES reach the default uncaught handler.
...
- [ ] **F3 — every block is a background `startActivity`, and the OS drops those silently.** Our only documented exemption is `SYSTEM_ALERT_WINDOW`, which F4 lets the user skip. (Since v1.17.1/v1.17.2 the `wasBlocked = true` row is written only AFTER `launchBlockOverlay` returns true and only once per arrival, so a dropped launch no longer records a block; the stale `isOverlayActive` flag half of this note still stands.) Detect cheaply (`Settings.canDrawOverlays` before every block); hardening means a `TYPE_APPLICATION_OVERLAY` fallback, for which the `CounterOverlayManager` machinery already exists.
- [x] ~~**F5 — a completed delay's passthrough survives screen-off.**~~ **RESOLVED in v1.16.0**, as part of the issue #28 sitting model. A `SittingTracker` now owns "has the user left", and a screen-off ends a sitting (`SittingEndCause.SCREEN_OFF`) exactly as Home does — revoking the grant, leaving session counts and the cooldown alone. Registered at runtime because `ACTION_SCREEN_OFF` is a protected broadcast; a failed registration degrades to the old behaviour, never to a false revoke. As the audit required, the fix is on screen-off and not on a timer, so #5 is not reintroduced: the GRANT still has no expiry, the SITTING does. **Refined in v1.18.1 (issue #54), and F5 still holds.** A screen-off no longer ends the sitting the instant the broadcast lands — it starts the same 2-minute away clock a foreign app window starts, and only an absence past that window ends the sitting. F5's repro is untouched because *unlock hours later* is far past the window; what stopped costing a fresh block is the case F5 never described, a display timeout while the user is still sitting in the app (Android blanks the screen on lack of input, not lack of attention). So a screen-off is no longer "exactly as Home does": **Home is still instant, a screen-off is now measured.** See `docs/architecture/accessibility-event-pipeline.md`.
