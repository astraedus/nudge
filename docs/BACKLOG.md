...
- [ ] **`isOwnAppWindowEvent` can never return true in production, so "the user opened Nudge" never clears the awareness overlays.** `NudgeAccessibilityService.shouldClearForOwnPackageEvent` tests `className?.startsWith(ownPackageName)`; the applicationId is `dev.astraedus.nudge` and the classes are `com.astraedus.nudge.*`, so the prefix never matches and the `clearOverlays(packageName, "own_app_window")` branch inside the `ForegroundSignal.OwnUi` handler is unreachable. Its unit tests pass because they supply their own `ownPackageName` (`PassthroughTest`'s `OWN_PACKAGE = "com.astraedus.nudge"`) — the trap recorded in `tasks/lessons.md` (2026-09-14). Cost is cosmetic (a counter or time-remaining overlay can linger over Nudge's own screens); it is not an enforcement hole. **Deliberately NOT fixed by v1.17.2's issue-#36 work**, which needed the neighbouring question "is this Nudge's own MAIN app window?" and answered it with a separate, positive, exact-class predicate (`isOwnMainAppWindowEvent`) rather than repairing this one under a change about counting. When fixing it: compare against the class prefix (`namespace`), not the applicationId, and add the assertion tying the test's package constant to the real `applicationId` so the next divergence fails a test instead of disabling a branch.
