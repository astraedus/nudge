...
rendered on all three overlays, and the press-and-hold that opens an app once its timer has run out.
**Read before touching `ui/overlay/`, `RecordWalkAwayUseCase`, `HoldToUnlock`, or `EmergencyPass*`.**
...
## HOLD TO UNLOCK, the timer ending hands the last step back to the user ([#35](https://github.com/astraedus/nudge/issues/35), v1.17.3)

> Read this before changing anything about how a timed block COMPLETES.

A user wrote in on 2026-09-18 and left the same request as a Play review the same week: when a block
timer ends, make the unlock a press-and-hold. They cite Wall Habit. The owner agreed to this item and
explicitly deferred the other two on the issue (per-app hold durations, uninstall protection).

The reason it is worth shipping is not the gesture, it is what the gesture changes about the cost of
entering. Before this, a `DELAY` or `BREATHING` block reaching zero **opened the app by itself**, so
the entire price of entry was *waiting*, and waiting is something a thumb can pay while the person
is somewhere else entirely. A hold is a few seconds the user has to keep choosing, and **letting go
is the cheap action**, which is the direction every affordance in this app should point.

### Shape

- **`domain/hold/HoldToUnlock.kt`**, pure Kotlin, no Android, no Compose.
  - `HoldToUnlock`: `OFF_SECONDS` (0), `DEFAULT_SECONDS` (3), `OPTION_SECONDS` (`0, 2, 3, 5`, what
    the Settings picker offers), `MAX_SECONDS` (30), `resolveDurationMs(ruleOverrideSeconds,
    globalSeconds)`, `isEnabled(durationMs)`.
  - `HoldProgress`, the state machine: `press`/`release`/`fraction`/`advance`/`reset`. `advance`
    returns true **exactly once** per completed hold and is the only thing between a sustained press
    and two passthrough grants. A release before completion abandons the attempt; a second `press`
    while already pressed RESTARTS from that moment, which is what makes time spent off-screen
    worthless (the same rule the countdown itself follows, issue #8). Unit-tested end to end in
    `HoldToUnlockTest`, because every interesting rule here is a rule about a gesture, and a gesture
    needs a finger and a stopwatch to observe on a device.
- **`ui/overlay/HoldToUnlockControl.kt`**, rendering and input only. It drives one `HoldProgress`
  from a `withFrameMillis` loop inside `repeatOnLifecycle(RESUMED)`, and calls the callback it was
  given when `advance` says the hold landed. One haptic tick at completion.
- **`DelayContent` / `BreathingContent`**, each takes `holdToUnlockMs`. When the timer finishes they
  set a remembered `timerFinished` instead of calling `onComplete()`, and render the hold control **in
  the place the countdown ring / breathing circle occupied**: one shape, two phases, nothing new to
  find on a screen the user is trying to leave. The headline and subtitle swap to "the wait is over"
  copy, since a motivational line is answering a question nobody is asking any more.
- **`BlockOverlayActivity`** resolves the duration once per delivery, in the SAME `runBlocking` that
  already reads the message pools and the emergency-pass state, for the same reason: the control
  replaces the countdown the instant it finishes, and a duration that arrived late would be a hold
  whose length changed while a thumb was already on it.

### The invariants, and why each one is pinned

- **The hold gates the EXISTING grant; it is never a second way in.** Both overlays pass their own
  `onComplete` straight through as the control's `onUnlock`, so completion still lands in
  `onTimerComplete()`—which is lifecycle-gated, because a countdown that reached zero off-screen
  was the issue #8 bypass. `HoldToUnlockContractTest` asserts `passthroughManager.grant` has exactly
  ONE call site in the activity, that it is inside `onTimerComplete`, and that the control file
  cannot reach `PassthroughManager` or `finish()` at all. This is the assertion the whole test file
  exists for: a press-and-hold that opens apps is one careless edit away from being an unguarded
  second door.
- **"Off" is the old behaviour, exactly.** `HoldToUnlock.isEnabled` decides, and with the hold off
  the timer calls `onComplete()` directly as it always did. A zero-length hold must never render—
  that would be an invisible dead end on a screen the user cannot leave except by walking away.
...
- **A re-delivered block starts from zero.** Nothing new was built for this: the machine is
  ...
  `render()` (issue #15), so a block arriving through `onNewIntent` discards it with everything else.
  Hooking into the same token is the point—a second freshness mechanism is a second thing to forget.
- **Walking away still works, and still costs nothing.** "I changed my mind" and the back gesture are
  untouched: the back callback lives on the activity and the button sits below the control, both live
  throughout the hold. Letting go is always free.
- **TalkBack can operate it.** A press-and-hold is invisible to a screen reader—TalkBack consumes
  touch exploration, so the gesture detector never sees a finger, and without a second path a blind
  user would be looking at the one control on screen that opens their app and be unable to use it.
  The control publishes a semantic click action that starts the SAME hold on the SAME machine for the
  SAME duration. The friction is the delay, not the finger, so nothing is given away.

### The preference, and the per-app override that is coming

One global `Int` of seconds (`hold_to_unlock_seconds`, default 3), picked in Settings under **Block
Screen**. It rides the backup file like every other configuration value, and an imported value is
bounded to `OFF_SECONDS..MAX_SECONDS` for the same reason the Strict Mode challenge length is: an
export file is plain, hand-editable JSON, and a hold nobody can physically complete is a permanent
lockout rather than a deliberate pause.

`resolveDurationMs(ruleOverrideSeconds, globalSeconds)` is the shape item 2 of the issue will fill
in. The call site already asks "what is the EFFECTIVE hold for THIS block" and passes `null` today,
so adding a nullable `holdSeconds` to `BlockRule` later changes one argument and nothing else.
Nothing is stored per-rule yet, so there is no migration to regret.

**The picker is deliberately NOT behind the Strict Mode unlock challenge**, and that decision matches
the one already made one section above it in Settings: changing the hold length does not undo any
block. The delay or the breathing exercise still runs in full, the app is still confronted, and the
walk-away is still there—exactly the reasoning that leaves the Strict Mode difficulty chips freely
editable. If that judgement is ever revisited, `SettingsWeakening` / `LockedToggle` is where it goes,
and both decisions should move together.

**Device QA pending.** The gesture's rules are unit-tested and its wiring is pinned at source level;
what no JVM test can see is whether the ring reads as something you press, whether 3 seconds feels
like friction or like a bug, and whether TalkBack announces the control usefully on a real phone.
