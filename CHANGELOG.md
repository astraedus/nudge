...
- **Opening a blocked app counts as one block, not two.** About half the time, opening a blocked app was recorded twice, so the Blocked number on your dashboard climbed roughly two and a half times faster than it should have. You never saw anything wrong on screen: the nudge screen took a moment to appear, and in that moment the app you had opened was still starting up behind it, which Nudge mistook for you having got past the block. It re-checked, blocked again, and wrote a second entry. Only the stats were affected, never the blocking itself, but every number built on them (the dashboard tile, the weekly chart, the apps that pull hardest, the Willpower page) was reading high. They will be accurate from now on; entries already recorded are left as they are.
