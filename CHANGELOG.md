...
### Added
- **Hold: a third block mode.** Instead of watching the countdown, you keep your finger on the screen for the whole time; let go and it starts over. Pick it in the rule editor next to Hard Block, Delay and Breathing, and set how long the same way you set a delay ([#35](https://github.com/astraedus/nudge/issues/35), asked for by a user who wrote in and left the same request as a review). It is the same wait, priced differently: a countdown is something a thumb can sit through while you are somewhere else, and a hold is time you have to keep choosing. Letting go is always free, "I changed my mind" and the back gesture work the whole way through, and it works with TalkBack too.
...
- **Walking away no longer counts as two blocks.** The Blocked number on the dashboard (and on the Today widget, and on the weekly chart under it) went up by two every time you turned around at a nudge screen, so a phone that had been stopped 181 times was reading 210. Nudge writes a second line in its history when you tap "I changed my mind" — that is how it knows you walked away — and the dashboard was counting that line as another block on top of the one it belonged to. The insight screens behind the tiles always got this right, which is why the two never quite agreed. Every "Blocked" number in the app now means the same thing: how many times Nudge actually stood in your way, walking away counted once, as a walk-away. Nothing in your history was lost or changed — the numbers over it were being added up wrong, and today they read lower and true.
...
- **Opening a blocked app counts as one block, not two.** Roughly half the time, opening a blocked app was recorded twice, so your Blocked count ran high (a measured run of ten openings recorded twenty-five blocks where twenty was correct). You never saw anything wrong on screen: the nudge screen takes a moment to appear, and in that moment the app you opened was still starting up behind it, which Nudge mistook for you having got past the block, so it checked again and wrote a second entry. Blocking itself always worked; it was only the counting. Every number built on it (the dashboard tile, the weekly chart, the apps that pull hardest, the Willpower page) was reading high and will be accurate from now on. Entries already recorded are left alone.
