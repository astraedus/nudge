...
- **The Protection widget tells you when blocking has actually stopped.** Android can kill Nudge's accessibility service on its own, and when that happens the app looks switched on while enforcing nothing. The widget turns red and says so on your home screen, without you opening anything. (One known rough edge: if you change the master switch within a few seconds of being blocked, the widget can take up to half an hour to catch up. The app itself is always right, and blocking is unaffected.)
